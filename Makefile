...
tests/cocotb/examples:
	$(QUIET)$(IN_ENV) env \
		CMD=$(PWD)/env/bin/verilator \
		SIM=$(SIM) \
		PYTHONDONTWRITEBYTECODE=1 \
		$(COCOTB_APPEND) \
		bash -c "cd cocotb ; make -C examples"

define cocotb_test_cases_cmd
tests/cocotb/$(1):
	$$(QUIET)$$(IN_ENV) env \
		CMD=$$(PWD)/env/bin/verilator \
		SIM=$(SIM) \
		PYTHONDONTWRITEBYTECODE=1 \
		$(COCOTB_APPEND) \
		bash -c "cd cocotb/tests ; make -C test_cases/$(1)"
endef

COCOTB_TEST_CASES = $(shell for p in cocotb/tests/test_cases/* ; do [ -d $$p ] && echo $$p | cut -d\/ -f4 ; done)
$(foreach t,$(COCOTB_TEST_CASES),$(eval $(call cocotb_test_cases_cmd,$(t))))
